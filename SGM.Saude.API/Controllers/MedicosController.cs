...
namespace SGM.Saude.API.Controllers
{
    ...
    public class MedicosController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetItemsEspecialidades()
        {
            var result = await _repository.GetAll().ConfigureAwait(true);

            if (result is null) { return NoContent(); }

            var listMedicos = new List<ConsultasEspecialidadesQuery>();
            foreach (var item in result.ToList())
            {
                listMedicos.Add(new ConsultasEspecialidadesQuery()
                {
