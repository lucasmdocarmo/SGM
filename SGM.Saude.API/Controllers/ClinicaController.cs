...
namespace SGM.Saude.API.Controllers
{
    ...
    {
        private readonly ICommandHandler<CadastrarClinicaCommand> _commandCadastrar;
        private readonly ICommandHandler<EditarClinicaCommand> _commandEditar;
        private readonly ICommandHandler<DeletarClinicaCommand> _commandDeletar;
        private readonly IClinicaRepository _repository;

        public ClinicaController(ICommandHandler<CadastrarClinicaCommand> commandCadastrar, ICommandHandler<EditarClinicaCommand> commandEditar, 
            ICommandHandler<DeletarClinicaCommand> commandDeletar, IClinicaRepository repository)
        {
        ...
        [HttpGet]
        [Authorize(Roles = "Clinica, Gestao")]
        ...
        [ProducesResponseType(typeof(Notification), StatusCodes.Status422UnprocessableEntity)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> UpdateItem([FromRoute][Bind] Guid id, [FromBody][Bind] EditarClinicaCommand command)
        {
        ...
        [ProducesResponseType(typeof(Notification), StatusCodes.Status422UnprocessableEntity)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> CreateItem([FromBody] CadastrarClinicaCommand command)
        {
            if (!ModelState.IsValid)
            {
                var notifications = new List<Notification>();
        ...
        {
            var command = new DeletarClinicaCommand() { Id = id };
