...
namespace SGM.Cidadao.API.Controllers
{
    ...
    {
        private readonly ICommandHandler<CadastrarImpostosCommand> _commandCadastrar;
        private readonly ICommandHandler<EditarImpostosCommand> _commandEditar;
        private readonly ICommandHandler<DeletarImpostosCommand> _commandDeletar;
        private readonly IImpostosRepository _impostosRepository;

        public ImpostosController(ICommandHandler<CadastrarImpostosCommand> commandCadastrar, ICommandHandler<EditarImpostosCommand> commandEditar,
            ICommandHandler<DeletarImpostosCommand> commandDeletar, IImpostosRepository impostosRepository)
        {
        ...
        {
            var result = await _impostosRepository.GetAll().ConfigureAwait(true);
        ...
        [ProducesResponseType(typeof(Notification), StatusCodes.Status422UnprocessableEntity)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> UpdateItem([FromRoute][Bind] Guid id, [FromBody][Bind] EditarImpostosCommand command)
        {
        ...
        [ProducesResponseType(typeof(Notification), StatusCodes.Status422UnprocessableEntity)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> CreateItem([FromBody] CadastrarImpostosCommand command)
        {
            if (!ModelState.IsValid)
            {
                var notifications = new List<Notification>();
        ...
        {
            var command = new DeletarImpostosCommand() { Id = id };
