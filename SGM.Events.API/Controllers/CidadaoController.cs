...
namespace SGM.Events.API.Controllers
{
    ...
    public class CidadaoController : ControllerBase
    {
        ...
        public CidadaoController(ProducerConfig config)
        {
            _config = config;
        }
        ...
        {
            return Created("Cidadao Criado", SendMessageByKafka(msg));
        }
            ...
            using (var producer = new ProducerBuilder<Null, string>(config).Build())
            {
                try
                {
                    var sendResult = producer
                                        .ProduceAsync("cidadao_topic", new Message<Null, string> { Value = message })
                                            .GetAwaiter()
                                                .GetResult();
